...
class Policy:
    def __init__(self, *args, **kwargs):
        ...
        print("policy using scope:", self.scope.name)
    ...
    def save(self, filename, exp, iteration=None):
        assert filename.endswith('.h5')
        with h5py.File(filename, 'w') as f:
            for v in self.all_variables:
                f[v.name] = v.eval()
            ...
            f.attrs['name'] = type(self).__name__
            f.attrs['args_and_kwargs'] = np.void(pickle.dumps((self.args, self.kwargs), protocol=-1))
            f.attrs['exp'] = np.void(pickle.dumps(exp), protocol=-1)
    ...
    def Load(cls, filename, extra_kwargs=None):
        with h5py.File(filename, 'r') as f:
            ...
            # print(argstring)
            # args, kwargs = json.loads(argstring)
            if extra_kwargs:
    ...
    @classmethod
    def LoadSnapshot(cls, filename, agent_id, extra_kwargs=None):
        import gym
        from . import policies, tf_util
        gym.undo_logger_setup()

        with h5py.File(filename, 'r') as f:
            exp = json.loads(f.attrs['exp'])
            config = es.Config(**exp['config'])
            thetas = f[str(agent_id)][:]

        env = gym.make(exp['env_id'])
        sess = es.make_session(single_threaded=True)
        policy = getattr(policies, exp['policy']['type'])(env.observation_space,
                                                          env.action_space,
                                                          **exp['policy']['args'])
        tf_util.initialize()
#        tf_util.initialize()
#            config, env, session, policy = es.setup_worker(exp, single_threaded=True)
            # policy needs to be made as above
            # policy = getattr(policies, exp['policy']['type'])(env.observation_space,
            #                                                   env.action_space,
            #                                                   **exp['policy']['args'])
        policy.set_trainable_flat(np.asarray(thetas))
        return config, env, sess, policy, exp, thetas
...
class MujocoPolicy(Policy):
    def _initialize(self, ob_space, ac_space, ac_bins, ac_noise_std, nonlin_type, hidden_dims, connection_type, same_param_start=False):
    ...
    def _make_net(self, o):
        ...
        if self.connection_type == 'ff':
            ...
            for ilayer, hd in enumerate(self.hidden_dims):
                ...
                x = self.nonlin(U.dense(x, hd, 'l{}'.format(ilayer), tf.zeros_initializer))
        else:
        ...
        if ac_bin_mode == 'uniform':
            ...
            a = U.dense(x, adim, 'out', tf.zeros_initializer)
        else:
