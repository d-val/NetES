...
source activate es-network
ln -s ~/collective-reinforcement-learning/networkx_graph_files ~/networkx_graph_files
#cp ./redis_config/redis_master.conf /etc/redis/redis.conf
...
sudo chown ubuntu:ubuntu /var/run/redis
...
""" % (make_disable_hyperthreading_script(), make_download_script(code_url), cmd)
...
def make_worker_script(code_url, master_private_ip):
    cmd = ("MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 "
           ...
           "--master_host {} ").format(master_private_ip)
    return """#!/bin/bash
{
set -x
...
@click.option('--archive_excludes', default=["aws_requests", "noncommittal", "configurations", "notes", "plots", "install", "video", "./logs", "*blob", ".git", "keys", "results", "writing", "logs", "final_plots"])
